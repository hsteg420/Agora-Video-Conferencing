//#1 client creation 'rtc': way to communicate with each other, codec: algorithm to encrypt videos
...
document.getElementById("leave-btn").addEventListener("click", async () => {
  ...
  document.getElementById("user-streams").innerHTML = "block";
...
let joinStreams = async () => {
  ...
  let player = `<div class="video-containers" id="video-wrapper-${config.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${config.uid}" src="./assets/mic-on.svg" /> ${config.uid}</p>
...
let handleUserJoined = async (user, mediaType) => {
  ...
  if (mediaType === "video") {
    ...
    player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="./assets/mic-on.svg" /> ${user.uid}</p>
